...
class Query(QuerySQL):
	...
	def init(func):
		...
		def ware(self, *args, **kwargs):
			if self.mode == 'sim_trade':
				if self.last_date != datetime.now().date():
					self.init_connect()	
					self.stock_info = None
					with open(DataPath+'/cache/query/components.pkl', 'rb') as f:
						self.components_data = pickle.load(f)
					self.last_date = datetime.now().date()
	...
	def get_price(self, security_list, field_list, start, end, unit='1m', fq=None, return_df=False):
		...
		assert set(field_list).issubset(set(['factor','open','high','low','close','volume'])), 'wrong field_list'
		assert len(security_list)<100, 'security_list larger than 100 use queryRAM'
	...
	def history(self, security_list, field_list, limit, unit='1m', fq=None, return_df=False, end=None):
		...
		if unit == '1W':
			...
			count = limit * 25
		elif unit in ['1D','2D','3D']:
			count = limit * int(unit[0])
		else:
