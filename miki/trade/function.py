...
import os, sys, pickle, pyecharts, time, bcolz, yaml
from miki.trade import glovar as g
import win32com.client


class MikiFunction(object):
	# 实现常见函数
	def __init__(self):
		super(MikiFunction, self).__init__()	
	@staticmethod	
	...
	def from_yaml(path):
		...
		return data		
	@staticmethod
	def notice(alert_str, alert_times=1):
		# 电脑语音提示
	...
	def reshape(df, unit):
		...
		if unit in ['5m','15m','30m','60m','120m']:		
			...
			df = df.groupby(lambda x:x.year+x.week*0.001)
		elif unit=='1M':
			df = df.groupby(lambda x:x.year+x.month*0.01)
		else:
			raise Exception('only 1m,5m,15m,30m,60m,120m,1d,1W,1M unit is available')
	...
	def show_kline(df):
		...
		kline.render(g.run_params['data_path']+'/cache/kline.html')
		os.startfile(os.path.abspath(g.run_params['data_path']+'/cache/kline.html'))
	@staticmethod
	...
	def plot_value(value_history, order_history, begin_cash, end_cash, save_path, print_result, open_file=False):
		...
		value_array,date_array,max_retrace,max_retrace_date,max_return,min_return,annual_rate = MikiFunction.calculate(value_history, \
	...
	def get_path(security, unit):
		...
		param1 += unit
		path = g.run_params['data_path']+'/{}/{}/{}'.format(param1,param2,security)				
		return path
	@staticmethod
	def get_security_list():
		# 获取股票代码
		stock_list = []
		for param in ['stock1m','fund1m']:
			for i in os.listdir(g.run_params['data_path']+'/'+param):
				for j in os.listdir(os.path.join(g.run_params['data_path']+'/'+param,i)):
					stock_list.append(j)
		# 沪深300、中证500基准
		stock_list = stock_list + ['399300.XSHE','000905.XSHG']
		return stock_list
	def get_dataframe(year, field, unit, update):
		# 获取某年的数据，主要用于回测推送
		start = pd.to_datetime('{}-01-01'.format(year))
		end = pd.to_datetime('{}-01-01'.format(year+1))
		cache_path = g.run_params['data_path']+'/cache/cache/{}-{}-{}.pkl'.format(field, unit, year)
		if not os.path.exists(cache_path) or update:
			print('not exist path {}, generate data !'.format(cache_path))
			security_list = MikiFunction.get_security_list()
			df_list = []
			for security in security_list:
				path = MikiFunction.get_path(security, unit=unit)			
